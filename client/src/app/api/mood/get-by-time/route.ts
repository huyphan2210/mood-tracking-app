...
export const getMoodByTime = () =>
  async function GET(req: NextRequest) {
    ...
    try {
      const response = await api.moodList(
        { startDate, endDate },
        {
          secure: true,
        },
      ...
      const moodInfo: MoodResponse = response.data
        ? await response.json()
        : null;

      const returnedResponse = NextResponse.json(moodInfo, {
        status: 200,
      });
...
export const GET = getMoodByTime();
